...
int main() {
    ...
    ponto p1(0.3,0.3,0.3); // Um ponto inicial
    double t;
    ponto grad;

    do {
        // Seu código aqui
        cout << "Temp= " << t << ", norm(grad)=" << grad.norm() << endl;

        break; // Remova esse break para rodar o programa 
    } while (grad.norm() > 0.01); // Condição de parada
    return 0;
}